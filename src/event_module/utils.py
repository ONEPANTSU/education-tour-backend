...
async def check_university_event(
    user_id: int, event_id: int, session: AsyncSession
):
    university_id = (await UserUniversityResponseHandler().get_by_filter(
        user_university_filter=UserUniversityFilter.USER, value=user_id, session=session
    )).data[UserUniversityDataKey().get(key="schema")].university_id
    events = (
        (
